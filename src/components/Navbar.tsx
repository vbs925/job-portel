        ...
        <div className="flex justify-between h-16 items-center">
          <div className="flex items-center gap-8">
            <Link href={user ? (user.role === 'MANAGER' ? '/manager/dashboard' : '/dashboard') : '/'} className="flex-shrink-0 flex items-center">
              <span className="text-xl font-bold text-foreground tracking-tight hover:text-primary transition-colors">
                TechCorp <span className="text-primary font-black">Careers</span>
              </span>
            </Link>
          </div>
              ...
              <>
                {user.role === 'APPLICANT' ? (
                  <>
                    <Link 
                      href="/dashboard" 
                      className="text-sm font-medium text-foreground/60 hover:text-foreground transition-colors flex items-center gap-2"
                    >
                      <LayoutDashboard className="w-4 h-4" /> Dashboard
                    </Link>
                    <Link 
                      href="/applications" 
                      className="text-sm font-bold text-foreground bg-secondary hover:bg-foreground/5 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2"
                    >
                ...
                <div className="relative group ml-4">
                  <div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold text-lg cursor-pointer shadow-sm hover:ring-2 hover:ring-primary/50 transition-all">
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-background border border-foreground/10 rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col overflow-hidden transform origin-top-right scale-95 group-hover:scale-100 z-50">
                    ...
                    <Link href="/profile" className="px-4 py-3 text-sm font-medium hover:bg-foreground/5 transition-colors flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className="px-4 py-3 text-sm font-medium text-red-500 hover:bg-red-50 transition-colors flex items-center gap-2 text-left w-full border-t border-foreground/5"
                    >
