...
export default function Applications() {
  ...
  const getStatusIcon = (stage: string) => {
    ...
    if (s.includes('screening') || s.includes('review')) return <Briefcase className="w-5 h-5 text-slate-600" />;
  ...
  const getStatusColor = (stage: string) => {
    ...
    if (s.includes('screening') || s.includes('review')) return 'bg-slate-600/10 text-slate-800 border-slate-200/50';
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-foreground/10 bg-background/50 backdrop-blur-md sticky top-16 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <Link href="/dashboard" className="inline-flex items-center gap-2 text-[14px] font-bold text-foreground/50 hover:text-foreground mb-4 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Dashboard
          </Link>
          <div className="flex justify-between items-end">
            <div>
              <h1 className="text-[30px] font-bold text-foreground tracking-tight">Application Tracker</h1>
            ...
            applications.map((app) => (
              <div key={app.id} className="border border-foreground/10 rounded-xl p-6 bg-background hover:border-foreground/30 transition-all hover:shadow-sm">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-[20px] font-bold text-foreground">{app.job?.title || "General Application"}</h3>
                    <p className="text-foreground/70 font-medium mt-1">{app.job?.company || "Open Submission"}</p>
                  </div>
                  <div className={`px-4 py-1.5 rounded-full border text-[14px] font-bold flex items-center gap-2 ${getStatusColor(app.stage)}`}>
                ...
                {app.stage?.toLowerCase().includes('interview') && (
                  <div className="mt-4 p-4 bg-purple-500/5 border border-purple-500/20 rounded-lg">
                    ...
                    <p className="text-[14px] text-foreground/70 mb-4">The hiring manager has requested an interview. Please select a time slot.</p>
                    <button className="px-4 py-2 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 transition-colors text-[14px]">
                      Select Time Slot
                    </button>
                  </div>
                )}
