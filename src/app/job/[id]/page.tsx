...
export default function JobDetailsPage() {
  ...
  // Very naive parser for the seed descriptions to split into sections
  const sections = job.description?.split('\n\n**') || [];
  const about = sections[0];
  const responsibilities = sections.find((s: string) => s.toLowerCase().includes('responsibilities:**'))?.replace('Responsibilities:**\n', '') || '';
  const requirements = sections.find((s: string) => s.toLowerCase().includes('requirements:**'))?.replace('Requirements:**\n', '') || '';
